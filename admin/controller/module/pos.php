...
//				}
	...
	public function createEmptyReturn($customer_id) {
		...
		if ($customer_id) {
			$this->load->model('sale/customer');
			$c_info = $this->model_sale_customer->getCustomer($customer_id);
			if ($c_info) {
				$data['customer_id'] = $customer_id;
	...
	public function modify_order($data) {
		...
		if ($action == 'insert' || $action == 'insert_quick') {
			...
			$this->db->query("INSERT INTO `" . DB_PREFIX . "order_product` SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$data['product_id'] . "', name = '" . $this->db->escape($data['product']['name']) . "', model = '" . $this->db->escape($data['product']['model']) . "', quantity = '" . (int)$data['quantity'] . "', price = '" . (float)$data['product']['price'] . "', total = '" . ((float)$data['product']['price'] * (int)$data['quantity'] * (float)$data['weight']) . "', tax = '" . $tax . "', reward = '" . $reward_points . "', weight = '" . (float)$data['weight'] . "'");
			...
			$json['text_total'] = $this->currency->formatFront(((int)$data['quantity']) * ((float)$data['weight']) * ($this->config->get('config_tax') ? $data['product']['price'] + $tax : $data['product']['price']));
		} elseif ($action == 'modify_quantity') {
			$quantity_change = (int)$data['quantity_after'] - (int)$data['quantity_before'];
			$sqlQuery = "UPDATE " . DB_PREFIX . "order_product SET quantity = " . (int)$data['quantity_after'] . ", total = price * weight * " . (int)$data['quantity_after'] . " WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . (int)$data['order_product_id'] . "'";
